  ...
  return (
    <>
      ...
      <div className="w-full py-5 px-3 flex items-center justify-between fixed top-0 left-0 z-[98] lg:px-12 lg:py-6">
        ...
        <nav className={`desktop_nav gap-14 hidden lg:${showSearch ? 'hidden' : 'flex'}`}>
          <a className="text-sm" href="#">Home</a>
          <a className="text-sm" href="#">About Us</a>
          <a className="text-sm" href="#">Shop</a>
          <a className="text-sm" href="#">Blogs</a>
        </nav>
        <div
          ref={searchRef}
          className={`w-[60%] h-8 flex justify-between items-center border border-white/30 bg-white/10 rounded-3xl lg:${showSearch ? 'flex' : 'hidden'} lg:w-[45%] lg:h-10 lg:px-3`}
        >
          <input
          ...
          />
          <img className={`w-4 mr-2 ${searchInput !== "" ? 'hidden' : 'block'}`} src={search} alt="search icon" />
          <img className={`w-4 mr-2 ${searchInput !== "" ? 'block' : 'hidden'}`} src={close} alt="close png" onClick={() => { setSearchInput('') }} />
        </div>
        <div className="flex items-center gap-3 lg:gap-7">
          <img className={`w-4 hidden lg:${showSearch ? 'hidden' : 'block'} lg:w-5`} src={search} alt="search icon" onClick={() => { setShowSearch(true) }} />
          <div className="relative">
            <img className="w-4 lg:w-6" src={cart} alt="cart icon" />
